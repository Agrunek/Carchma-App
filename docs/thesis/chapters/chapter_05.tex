...
Rejestracja konta w serwisie odbywa się poprzez formularz, gdzie użytkownik podaje podstawowe informacje, takie jak adres e-mail, na który jest wysyłany link aktywacyjny w celu sfinalizowania procesu \figref{fig:registerpage}:
%
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{register_page}
    \figcaption{Forumlarz rejestracyjny użytkownika}{Opracowanie własne}
...
System ten wspiera mniej doświadczonych kupującym na bardziej przemyślane decyzje, dzięki opiniom użytkowników o większym doświadczeniu, a jednocześnie pozwala na sprawniejszą identyfikację nieuczciwych ofert \figref{fig:commentsview}:
%
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{comments_view}
    \figcaption{Przykładowa sekcja komentarzy pod ogłoszeniem}{Opracowanie własne}
    \label{fig:commentsview}
\end{figure}

\section{System zgłoszeń dla ofert i komentarzy}

Zarówno oferty, jak i komentarze mogą być zgłoszone przez użytkowników za pośrednictwem dedykowanego przycisku.
...
Wybór konkretnego kafelka skutkuje przekierowaniem użytkownika do osobnej strony, zawierającej szczegółowe informacje dotyczące danej oferty \figref{fig:searchpage}:
%
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{search_page}
    \figcaption{Wyszukiwarka w aplikacji}{Opracowanie własne}
...
Responsywność zapewnia, że wszystkie składowe serwisu są czytelne i wygodne w użytkowaniu niezależnie od platformy, na którym jest uruchamiany \figref{fig:mobile}.

\section{Dostępność}

Każda z funkcjonalności serwisu została zaprojektowana z możliwością obsługi klawiaturowej bez konieczności używania kursora myszy.
Kolory tekstu oraz tła zostały dobrane w taki sposób aby spełniać minimalne wymagania kontrastu WCAG \autocite{wcag_guidelines}.
Wszystkie komponenty automatycznie dostosowują swoją kolorystykę zależnie od wybranego przez użytkownika motywu, jasnego lub ciemnego \figref{fig:mobile}.
...
\begin{figure}[h]
    ...
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=0.9\linewidth]{mobile_light}
        \caption{Tryb jasny}
        \label{fig:mobilelight}
    \end{subfigure}
    %
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=0.9\linewidth]{mobile_dark}
        \caption{Tryb ciemny}
        \label{fig:mobiledark}
    \end{subfigure}

    \figcaption{Widok na urządzeniu mobilnym}{Opracowanie własne}
    \label{fig:mobile}
\end{figure}
